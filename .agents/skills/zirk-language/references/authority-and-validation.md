...
is not a passing fixture by itself. If the checkout binary cannot be run, state that the
...
| --- | --- |
| Bindings, scalar types, literals, nullability, conversions | `foundations.md`, `CORE_LANGUAGE_SEMANTICS.md`, matching handbook chapter |
| Arrays, lists, ranges, indexing, slices, iteration | `collections-and-iteration.md`, `CORE_LANGUAGE_SEMANTICS.md`, handbook collections/iteration chapters |
| Control flow, `match`, functions, lambdas | `control-flow-and-callables.md`, `CORE_LANGUAGE_SEMANTICS.md`, matching handbook chapter |
| Classes, records, enums, contracts, generics | `data-modeling-and-generics.md`, `CORE_LANGUAGE_SEMANTICS.md`, matching handbook chapter |
| Dates, zones, durations, parsing | handbook `03a-temporal/` and `ZIRK_STDLIB_SPEC.md` §8 |
| `Result`, `throws`, `try`, resources, manifest permissions | `failures-resources-and-authority.md`, failure/permission semantics plus handbook `15-errors`, `16-resources`, `03-projects` |
| Pointers and C ABI | memory/unsafe semantics plus handbook `17-memory-and-safety` and `05-native-and-low-level` |
| A concrete `std.*` API | handbook `04-standard-library` chapter plus `ZIRK_STDLIB_SPEC.md`; verify its signature, errors, and permission |
| Decorators or reflection | decorator semantics plus handbook `06-metaprogramming` |
| Modules, packages, project layout, dependencies | `projects-modules-and-validation.md`, handbook `19-modules`, `03-projects`, `09-packages`, plus `ZIRK_COMPILER_SPEC.md` |
| Tests, CLI, formatting, diagnostics | `projects-modules-and-validation.md`, handbook `08-testing`, `07-toolchain`, and `ZIRK_COMPILER_SPEC.md` §§8–10 |

## Explicit exclusions that must not be improvised

Zirk 1.x excludes WebAssembly/browser DOM integration, `async fn`, a public
textual inline assembly, general
...
(`match with`, generics/unions/distinct names, C ABI wrappers, etc.) or
