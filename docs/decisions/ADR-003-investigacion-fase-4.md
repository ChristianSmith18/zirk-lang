...
Nota sobre un bug encontrado al escribir la primera versión de este probe (no corregido, fuera de alcance): la versión inicial usaba `counter: Counter?` y llamaba `c?.bump()` dentro de un brazo de `match`. Eso hizo **panicar** la compilación en `crates/zirk-ir/src/lower.rs:4660`, con el mensaje *"a method reachable through `?.` has a nullable form"* — el código intenta envolver el tipo de retorno del método en `Nullable::of(returns)` para el resultado de la llamada segura, y falla cuando ese tipo de retorno es `Void` (`Nullable::of` no puede envolver `Void`). Es decir: **llamar, a través de `?.`, un método cuyo tipo de retorno es `Void`, panica la codegen hoy**. No está en la lista de 5 bugs del documento original porque no se había ejercitado ese caso concreto (`?.` sobre una llamada de método sin valor de retorno). Se documenta aquí, sin intentar corregirlo, para que quede asentado junto a los otros.
