...
| `parallel` | yes | yes | yes | yes | yes | yes | `parallel-cpu-regions`: `parallel { }` (`;`-separated `cores`/`chunk` header) opens a CPU region on a new worker pool (`crates/zirk-runtime/src/pool.rs`), separate from the cooperative executor. A `for x in coll { }` filling the whole region body, over `Array`/`List`, with no `return`/`break`/`continue`/`throw` and no reassignment of a captured name, work-splits across the pool (IR: `InstKind::ParallelForStart`; codegen: `build_parallel_for_thunk`; runtime: `zirk_rt_parallel_for`); anything else in a region runs sequentially, inline. `Timer.*`/`spawn`/`concurrent {}`/printing are rejected inside a region (`PARALLEL_REGION_IO`); a captured `mut` reference type at the boundary is rejected (`STRICT_ALIAS_VIOLATION`). Multi-threaded stop-the-world safepoint collector delivered alongside it (`ADR-019`). Found and fixed a real, pre-existing GC-rooting bug along the way (`Array`/`List`/`Range`/`Map`/`Set` locals were never actually walked by the collector — see `STRUCTURED_CONCURRENCY_SEMANTICS.md` §8 / `MEMORY_AND_UNSAFE_SEMANTICS.md` §14). **Not delivered**: `.parallel`/`Parallel.each` are typed but not real adapters — no `map`/`filter`/`reduce`/etc. exists on `List<T>`/`Array<T>` at all yet, sequentially or otherwise, so there is no pipeline for them to parallelize; a parallel-reduce associativity check is blocked on the same gap. `thread`/`Sync` remain gated (`Keyword::Thread`/`Sync`, Phase 5's own `concurrency-completion`). |
