...
| `Date` / `Time` / `DateTime` (civil temporal) | yes | yes | yes | yes | yes | yes | `date-and-time-types`: validated `Date(y,m,d)`/`Time(h,m,s?,ns?)`/`DateTime(d,t)` constructors throwing `InvalidDateError`/`InvalidTimeError`, component properties, ISO `to_string()`, `Date ± Int`/`Date - Date`/`Time ± Duration`/`Date + Time`, and `Date.today()`/`Time.now_local()`/`DateTime.now_utc()`/`now_local()`. |
| Zone-aware temporal family (`Instant`, `ZonedDateTime`, `TimeZone`, `Period`) | yes | no | no | no | no | no | `pending_type` Phase 7; `date-and-time-types` delivered the civil subset only. |
