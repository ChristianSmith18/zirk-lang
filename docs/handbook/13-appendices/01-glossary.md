...
- **Final-result scalar conversion:** an explicit native conversion `T(expression)` that evaluates `expression` under its ordinary rules and converts only its completed result. The target type never propagates into nested operators or operands.
