...
Zirk retains Python-like String repetition (`"ja" * 3`) and expressive slicing,
but String is grapheme-indexed and mutation is permission-controlled. Integer
division with `/` is mathematical and returns exact `Decimal`; an explicit
`IntN(...)` converts the completed quotient when an integer is required.
Numeric mixing and conversion are statically checked, and temporal meanings
use distinct types rather than one general-purpose datetime object.
