...
| collections and generics | defined | native iteration subset exists; `array-list-tuple-duration-regex` adds `Array<T>` (fixed-capacity) and `List<T>` (resizable) as native reference collections with indexed read/write, bounds checking, `length`, `is_empty`, and `for ... in` through `Iterable<T>`, plus `Range<T>` (`start..end`, `start..end..step`, `.reverse()`, slicing) for numeric `T` and `Duration` | `Map<K,V>`/`Set<T>`, complete stdlib collections and general generic contract/enum delivery |
...
| tuples, regex literals and basic matching | defined | `array-list-tuple-duration-regex` delivered tuple literals, `[N]` indexing, `match` destructuring, `Duration` suffixes/arithmetic/printing, `re'...'` with `matches`/`find`/`replace`, the `String` surface (`s[i] = c` write, `[start:end:step]` slicing, `trim`/`search`/`contains`/`starts_with`/`ends_with`/`substring`/`split`), `Char` classification/normalization, `Regex.split`, `Regex.find_all` match iteration, `re'...'` patterns in `match`, `Range<T>`, `type` alias lowering, derived `Clone` for `record`/`enum`, and generic contract `implements Contract<T>` satisfaction for contracts whose members do not name their own type parameter | `Range<T>` beyond `Int32`/`Duration` elements, contract members that name the contract's own `T`, and the wider per-type method surface the API chapters mark `specified` (scalar math/checked/wrapping/saturating families, `String` case/replace/length, `Char` Unicode classification beyond the delivered subset, collection `clone`/`to_string`/literal forms, `Regex.parse`, the universal `.type` member, `Duration` sign/magnitude methods) |
