...
The type taxonomy, Float and BinaryFloat families, grapheme Char, shared mutable String, binding
...
| scalar and text families | defined | Phase 3b complete for its scope | `BinaryFloat128` Windows verification; `BinaryFloat128` formatting truncates to `BinaryFloat64` (lossy) |
| classes, contracts and records | defined | Phase 3 scoped implementation; `fase-3-abstract-dispatch` delivered dynamic dispatch through a user-declared `abstract class`, generalizing the vtable mechanism already proven by the native `Throwable` hierarchy; `fase-3-structural-equality` delivered derived `==`/`!=` for `record` (field-by-field, short-circuiting, recursing into nested `record` fields); `fase-3-generic-enums` delivered lowering for a flat single/multi-type-parameter user generic enum; `fase-3-generic-substitution-recursion` fixed generic-inference substitution to recurse into a nested generic instantiation (a gap shared by every generic function/method call, not enum-specific); `fase-3-recursive-enums` made enum declaration order independent of self/mutual references (an enum and a class, or two enums, can now reference each other regardless of order) and turned the genuinely-self-referential-field hazard (a real compiler crash found during that work) into a clean compile-time diagnostic; `fase-3-value-type-contract-dispatch` delivered dispatch through a contract-typed reference for a `record` implementing it, by boxing into an ordinary collector-tracked allocation with a descriptor built the same way a class's already is — `CallContract` itself needed no changes; `array-list-tuple-duration-regex` removed `value class` from the language (migrate to `record`), lowered `type` aliases so they are usable in executable programs, delivered derived `Clone` for `record`/`enum` and lowered generic contract `implements Contract<T>` satisfaction for contracts whose members do not name their own type parameter | contract members that name the contract's own `T` (the substitution pass a call like `put(x: T)` through `Contract<T>` would need), constructing/using a genuinely self-referential enum field (needs automatic heap indirection — a new `IrType` case plus runtime/GC integration — not yet designed) |
...
| collections and generics | defined | `array-list-tuple-duration-regex` added `Array<T>` and `List<T>` with indexed read/write, bounds checking, `length`/`is_empty`, and `for ... in`; `native-type-member-surface` added `Array`/`List` literals, negative indexing, `Array` slicing, `clone()`, `to_string()`, and `List.remove(value)` | `Map<K,V>`/`Set<T>` and general generic contract/enum collection delivery |
| temporal family | defined | `Duration` literals, arithmetic and printing delivered; other civil/zone types remain at syntax/type design | runtime and `std.time` delivery for remaining temporal types in Phase 7 |
| tuples, regex literals and basic matching | defined | `array-list-tuple-duration-regex` delivered tuple literals, `[N]` indexing, `match` destructuring, `Duration` suffixes/arithmetic/printing, `re'...'` with `matches`/`find`/`replace`, the `String` surface (`s[i] = c` write, `[start:end:step]` slicing, `trim`/`search`/`contains`/`starts_with`/`ends_with`/`substring`/`split`), `Char` classification/normalization, `Regex.split`, `Regex.find_all` match iteration, `re'...'` patterns in `match`, `Range<T>`, `type` alias lowering, derived `Clone` for `record`/`enum`, and generic contract `implements Contract<T>` satisfaction for contracts whose members do not name their own type parameter; `native-type-member-surface` added scalar math/checked/wrapping/saturating families, `Float.format(spec)`, `String` `length`/`byte_length`/`is_empty`/`find`/`replace`/`trim_start`/`trim_end`/`to_lowercase`/`to_uppercase`/`normalize`/`split_whitespace`/`lines`/`bytes`/`codepoints`/`chars`/`clone`, `Char` `byte_length`/`codepoint_count`/`ascii_code`/`is_ascii`/`is_alphabetic`/`is_numeric`/`is_alphanumeric`/aliases/normalization, `Regex.parse`, `Duration` sign/magnitude methods, and default `to_string()` for tuples, records, classes, algebraic enums and callable values | `Range<T>` beyond `Int32`/`Duration` elements, contract members that name the contract's own `T`, the universal `.type` member, `Char` `bytes()`/`codepoints()` views, and the wider per-type method surface the API chapters mark `specified` |
