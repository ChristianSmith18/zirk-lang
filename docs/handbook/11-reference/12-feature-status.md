...
| memory, native views and transactional unsafe | defined | object allocation is intentionally transitional; `inmut::strict` rejects a direct rebinding and a write through a field projection off a strict binding | Phase 4e strategy delivery (ADR-003 still open), dependent references, field-declared strictness across a projection boundary, a mutating method call through a strict reference, rollback/commit |
