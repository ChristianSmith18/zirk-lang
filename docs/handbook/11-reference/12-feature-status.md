...
| memory, native views and transactional unsafe | defined | ADR-003 closed: non-moving mark-sweep, real, reclaiming unreachable memory including cycles (`zirk_rt_alloc` no longer "never frees"); `Weak<T>` delivered (`Weak.from`, `.upgrade(): T?`, `.is_alive`, never keeps its referent alive, cleared before the referent is freed); `inmut::strict` rejects a direct rebinding and a write through a field projection off a strict binding; `unsafe fn`/`unsafe {}`/`commit {}` parse and are context-checked; `Pointer<T>` (ABI-safe element types) supports construction/read/write/offset/cast with a conservative escape rule; `extern "C" fn` declares and calls a native function under a narrow ABI-safe signature, resolved by the system linker | deep `clone()`, dependent references and automatic pinning, field-declared strictness across a projection boundary, a mutating method call through a strict reference, the transactional journal/rollback itself (writes are not yet journaled or rolled back), `NativeSlice<T>`/volatile access, a native-library-linking manifest |
