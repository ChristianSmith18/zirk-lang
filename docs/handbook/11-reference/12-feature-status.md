...
| tuples, regex literals and basic matching | defined | `array-list-tuple-duration-regex` delivered tuple literals, `[N]` indexing, `match` destructuring, `Duration` suffixes/arithmetic/printing, `re'...'` with `matches`/`find`/`replace`, the `String` surface (`s[i] = c` write, `[start:end:step]` slicing, `trim`/`search`/`contains`/`starts_with`/`ends_with`/`substring`), `Char` classification/normalization, `Regex.split`, `Regex.find_all` match iteration, `re'...'` patterns in `match`, `Range<T>`, `type` alias lowering, derived `Clone` for `record`/`enum`, and generic contract `implements Contract<T>` satisfaction for contracts whose members do not name their own type parameter | `String.split`, `Range<T>` beyond `Int32`/`Duration` elements, contract members that name the contract's own `T` |
