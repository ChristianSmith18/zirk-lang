...
| memory, native views and transactional unsafe | defined | object allocation is intentionally transitional; `inmut::strict` rejects a direct rebinding and a write through a field projection off a strict binding; `unsafe fn`/`unsafe {}`/`commit {}` parse and are context-checked; `Pointer<T>` (ABI-safe element types) supports construction/read/write/offset/cast with a conservative escape rule; `extern "C" fn` declares and calls a native function under a narrow ABI-safe signature, resolved by the system linker | Phase 4e strategy delivery (ADR-003 still open), dependent references, field-declared strictness across a projection boundary, a mutating method call through a strict reference, the transactional journal/rollback itself (writes are not yet journaled or rolled back), `NativeSlice<T>`/volatile access, a native-library-linking manifest |
