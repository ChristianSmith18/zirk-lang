...
| memory, native views and transactional unsafe | defined | ADR-003 closed: non-moving mark-sweep, real, reclaiming unreachable memory including cycles (`zirk_rt_alloc` no longer "never frees"); `inmut::strict` rejects a direct rebinding and a write through a field projection off a strict binding; `unsafe fn`/`unsafe {}`/`commit {}` parse and are context-checked; `Pointer<T>` (ABI-safe element types) supports construction/read/write/offset/cast with a conservative escape rule; `extern "C" fn` declares and calls a native function under a narrow ABI-safe signature, resolved by the system linker | `Weak<T>`, deep `clone()`, dependent references and automatic pinning (a live/dead distinction now exists for these to build against, none are implemented yet), field-declared strictness across a projection boundary, a mutating method call through a strict reference, the transactional journal/rollback itself (writes are not yet journaled or rolled back), `NativeSlice<T>`/volatile access, a native-library-linking manifest |
