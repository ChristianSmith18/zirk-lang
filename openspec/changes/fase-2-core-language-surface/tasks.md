...
- [ ] 1.5 Retirar del diagnóstico "construcción de fase posterior" las palabras `for`, `while`, `loop`, `match`
- [ ] 1.6 Tests: un caso válido y uno inválido por cada regla nueva

## 2. Gramática — funciones completas y closures

- [ ] 2.1 Parsear parámetros opcionales (`nombre?: Tipo`)
- [ ] 2.2 Parsear valores por defecto en parámetros
- [ ] 2.3 Parsear el parámetro variadic (`...nombre: Tipo`), rechazando que no sea el último
- [ ] 2.4 Parsear argumentos nombrados en llamadas
- [ ] 2.5 Parsear lambdas de expresión y de bloque (D4)
- [ ] 2.6 Tests: un caso válido y uno inválido por cada regla nueva

## 3. Gramática — `match` y enum mínimo

- [ ] 3.1 Parsear `enum Nombre { Constructor, ... }` sin datos asociados (D1)
- [ ] 3.2 Parsear `match` como sentencia y como expresión, con brazos `patrón => cuerpo`
- [ ] 3.3 Parsear patrones: literal, constructor de enum, binding, `_`
...
- [ ] 5.1 Parsear el modificador `share` sobre declaraciones de nivel superior
- [ ] 5.2 Parsear `import { nombres } from "ruta"` con rutas locales entre comillas
- [ ] 5.3 Parsear alias de importación (`nombre -> alias`)
- [ ] 5.4 Parsear `import { nombres } from modulo.estandar;` sin comillas
- [ ] 5.5 Parsear `use nombre;`
- [ ] 5.6 Retirar el diagnóstico de Fase 1 que rechazaba todo `import`
- [ ] 5.7 Tests: un caso válido y uno inválido por cada regla nueva
