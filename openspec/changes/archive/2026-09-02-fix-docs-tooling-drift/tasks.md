...
- [x] 3.1 Edit `scripts/check-local.sh` to insert `cargo build --workspace` (with a `== build ==` section message, following the existing `blue`/`green` style) immediately before the `== tests ==` section.
- [x] 3.2 Verify, in a clean working tree (disposable copy with `target/` removed), that `./scripts/check-local.sh` builds before testing and that the tests in `crates/zirk-cli/tests/end_to_end.rs` find the `zirk` executable and the static runtime without an "executable was not found" error.
- [x] 3.3 Confirm that the order and flags of `scripts/check-local.sh` remain equivalent to the "Formato", "Clippy", "Build", "Tests" steps of `.github/workflows/ci.yml`.

## 4. Fix the checker's pending-type table

- [x] 4.1 Locate the `Iterable`, `Iterator`, and `Resource` entries in `pending_type` inside `crates/zirk-sema/src/types.rs` and confirm that `crates/zirk-sema/src/checker.rs` already registers them as resolved native contracts (lines cited in the survey: `checker.rs:958`, `checker.rs:1499`).
- [x] 4.2 Remove those three entries from `pending_type`.
- [x] 4.3 Add a regression test in `crates/zirk-sema/tests/typing.rs` verifying that annotating a type with `Iterable<T>`, `Iterator<T>`, or `Resource<E>` does not produce a pending-phase diagnostic.
- [x] 4.4 Run `cargo test -p zirk-sema` and confirm the existing 412 tests in `typing.rs` stay green alongside the new one.

## 5. Full verification

- [x] 5.1 Run `export LLVM_SYS_201_PREFIX=<LLVM 20.1 prefix> && cargo fmt --all --check`.
- [x] 5.2 Run `cargo clippy --workspace --all-targets -- -D warnings`.
- [x] 5.3 Run `cargo build --workspace`.
- [x] 5.4 Run `cargo test --workspace -- --test-threads=1` and confirm 995+1 (the new test from 4.3) tests green, 0 failing.
- [x] 5.5 Re-read `README.md`, `docs/decisions/README.md`, `docs/init/ZIRK_ROADMAP.md`, `docs/init/ZIRK_AGENT_PROMPT.md`, and `docs/init/ZIRK_FEATURE_STATUS.md` together and confirm none contradicts another.

## 6. Sync with the public website

- [x] 6.1 Commit this change's edits in `zirk-lang`.
- [x] 6.2 Run `./scripts/sync-website-content.sh` from `zirk-lang` pointing at `../zirk-lang-site`.
- [x] 6.3 Review the generated diff in `../zirk-lang-site` (site's own status catalog, hashes, handbook-imported content).
- [x] 6.4 If public status evidence changed (current phase, limitations, publicly cited ADRs), approve the review with an explicit `--audit-date YYYY-MM-DD` and record who reviewed it.
- [x] 6.5 Commit the result in `zirk-lang-site` separately, and note the corresponding `zirk-lang-site` revision/commit in the `zirk-lang` PR description.

## 7. Record the engineering debt not resolved here

- [x] 7.1 Create a follow-up note (issue, or an entry in `docs/decisions/proximos-pasos-fase-4.md` if applicable, or a new backlog document) for: `String`/`Char` lifetime strategy outside the mark-sweep collector, with a suggested change name (e.g., `fase-4e-memoria-de-strings`).
- [x] 7.2 Record the same way: audit of `static mut`/`thread_local!` in `zirk-runtime` ahead of starting Phase 5 (e.g., `fase-4e-runtime-thread-safety`).
- [x] 7.3 Record the same way: modularizing `crates/zirk-ir/src/lower.rs` and `crates/zirk-codegen-llvm/src/emit.rs` (e.g., `refactor-lower-y-emit`).
- [x] 7.4 Record the same way: the `value class` versus `record` scope decision, and semantic verification of `in`/`out` variance (e.g., `fase-3-value-class-alcance`, `fase-3-verificacion-varianza`).
- [x] 7.5 Link the four follow-up notes from `docs/init/ZIRK_FEATURE_STATUS.md` or from `COMPILER_IMPROVEMENT_SUGGESTIONS.md`, so they are not lost as loose comments in this change.
