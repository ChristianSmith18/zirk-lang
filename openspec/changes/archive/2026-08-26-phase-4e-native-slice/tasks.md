...
- [x] 0.1 Add `Expr::Index { receiver: Box<Expr>, index: Box<Expr> }` to `crates/zirk-ast` (check the actual current shape of `Expr::Field`/`Expr::Call` first and match that pattern/field-naming convention, don't invent a divergent shape).
- [x] 0.2 Parser: recognize `expr '[' expr ']'` as a postfix expression at the same precedence tier as method call/field access, left-associative, chainable (`a[i][j]`, `a.field[i]`). The `[`/`]` tokens are already lexed (`crates/zirk-lexer`) — this is purely a new parser production, no lexer change needed.
- [x] 0.3 Assignment-target/place classification: extend whatever mechanism already classifies `expr.field` as a writable place (`zirk-type-system`'s "Projection copy and whole-reference aliasing" territory) to also classify `Expr::Index` as a place — `view[0] = 0x7f;` must parse and be usable as an assignment/simultaneous-assignment destination.
- [x] 0.4 Parser/AST tests: `expr[index]` parses as `Expr::Index`; it is accepted on the left side of `=`; `a.field[i][j]` parses as the expected left-associative chain.

## 1. Checker (zirk-sema)

- [x] 1.1 Add `Base::NativeSlice(u32)`/`Base::NativeSliceMut(u32)` (ids into an interning table, matching `Base::Pointer(u32)`'s actual current shape — verify, don't assume, per `fase-4e-weak`'s own precedent of checking first). Restrict element type `T` to the same ABI-safe subset `Pointer<T>` already accepts (verify the actual current list in the checker, not just design's restated summary of it).
...
- [x] 4.2 Runtime unit tests: valid construction succeeds; null pointer rejected; misaligned pointer rejected; a `length` exceeding a known `zirk_rt_alloc`'d buffer's real size rejected.

## 5. Cross-cutting correctness tests (do not skip)

- [x] 5.1 End-to-end `.zrk` fixture: construct a `NativeSlice<T>`/`NativeSliceMut<T>` from a validated `Pointer<T>`, read/write through it in safe code (outside any `unsafe` block) using `view[i]`/`view[i] = x` syntax — confirm this actually compiles and runs without requiring `unsafe` at the use site, only at construction.
- [x] 5.2 End-to-end `.zrk` fixture: an out-of-range index on a constructed view fails with a controlled bounds error, not a crash or silent out-of-bounds access.
- [x] 5.3 End-to-end `.zrk` fixture: `pointer.as_slice(length)` with a `length` that exceeds the real underlying allocation returns `Error`, not a view that would allow out-of-bounds reads.
- [x] 5.4 Compile-time rejection fixture: returning a `NativeSlice<T>` whose owner is a `Pointer<T>` local to the function fails to compile — the "Native view escapes its borrow" scenario, exercised for real (this scenario was already normative but unenforced before this change; confirm it is now actually enforced, not just documented).
- [x] 5.5 Compile-time rejection fixture: writing through a read-only `NativeSlice<T>` (`view[0] = x` where `view: NativeSlice<T>`, not `NativeSliceMut<T>`) fails to compile.
- [x] 5.6 Ran `LLVM_SYS_201_PREFIX=/opt/homebrew/opt/llvm@20 cargo test --workspace`; `cargo clippy --workspace --all-targets` clean; `cargo fmt --check` clean.
