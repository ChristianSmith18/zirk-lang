...
`docs/decisions/ADR-003-memoria.md`'s "Cierre de la decisión" (this session) already fixed the strategy: non-moving mark-sweep, function-granularity shadow stack, cooperative allocation-triggered collection. This document covers the implementation-level decisions that ADR text doesn't (or restates the ones it does, at the granularity code needs).
...
Alternative considered: steal a bit from the existing descriptor word instead of adding a `next` field. Rejected — every one of the three runtime functions above, plus every codegen site that dispatches through the descriptor, would need to mask the bit out; touching all of them to save one word per object is a bad trade, and `ADR-012` explicitly reserved header growth for exactly this situation ("lo que la Fase 4 necesite... se añade a la cabecera").
