...
- [ ] 2.2 Lower trait default bodies under the substitution context (pending: default bodies for `T`-bearing contract methods still hit `unreachable!`).
- [x] 2.3 Emit `CallContract`/`SafeDispatch::Contract` for calls through generic contract references.
- [~] 2.4 Add a substitution recursion guard producing a controlled diagnostic instead of stack overflow: nested generic `implements` arguments are rejected with `E0423`; a finer runtime guard is pending.

## 3. Backend

- [x] 3.1 Verify itable lookup in `emit.rs` covers generic instantiations; extend if the current scheme misses them.
- [x] 3.2 Confirm vtable offset stability across `Box<Int32>` and `Box<String>` (spec: substitution does not corrupt the vtable).

## 4. Fixtures and status

- [x] 4.1 Extend/add valid corpus fixtures: generic class + generic record implementing `Container<T>`, call through contract reference, two adopters sharing offsets.
- [x] 4.2 Add invalid fixtures: missing method under substitution, recursive generic `implements` guard.
