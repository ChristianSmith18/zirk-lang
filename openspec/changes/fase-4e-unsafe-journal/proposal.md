...
- **Journaling writes through a `NativeSlice<T>`/`NativeSliceMut<T>` view** — that type doesn't exist yet (`fase-4e-native-slice`, parallel work); this change's journal only guards `Pointer<T>`/managed-slot writes already lowered today.
