...
## Addendum: D2 was wrong — closed with a safety net, not a fix

Implementation found D2's own prediction false: a directly self-referential enum field is not a `specialize_enum` memoization question, because nothing recursive ever reaches that pass — every enum lowers to an inline-flattened struct with no indirection anywhere in the pipeline (`IrType::is_managed_reference`, `EnumLayout`, and every codegen field-type computation all recurse on the documented, invalidated assumption that "a layout can never nest itself"). Confirmed against a real build: lifting the old `E0402` gate alone crashes the compiler with a stack overflow.

### D3: Reject the unindirected shape at the checker, as a graph problem over all declared enums, not a per-declaration check

`Checker::reject_unindirected_enum_cycles`, run once after every enum's variants are resolved (`declare_members`'s own final step): a depth-first search over "variant field names enum" edges, starting from each enum in turn, following only edges where the named type is itself another enum (`Base::Enum`/`Base::EnumInstance`) — a field of any other type (`Object`, `Contract`, `Pointer<T>`, `Weak<T>`) already carries a heap indirection that breaks the cycle by construction, so it is never followed. If the search revisits its own starting enum, that enum is rejected with `NOT_LOWERED`, naming the specific cycle (direct self-reference, or a longer cycle through another enum).

This is deliberately a *graph* search across every enum, not a check scoped to one declaration at a time — a cycle through two or more distinct enums (enum A's variant embeds enum B inline, B's own variant embeds A inline) hits the exact same infinite-size hazard as direct self-reference and needs the same rejection, even though neither enum alone looks self-referential from its own declaration.

Alternative considered: defer this check to IR lowering (catch it where `is_managed_reference`'s own recursion would otherwise overflow) rather than at the checker. Rejected — the checker already has full type information for every declared enum by the point `declare_members` finishes, and every other checked-but-not-yet-compilable construct in this codebase is already rejected at the checker with `NOT_LOWERED`, not deep in IR/codegen; matching that existing convention keeps the diagnostic clean and immediate rather than a stack-trace-shaped surprise several passes later.
